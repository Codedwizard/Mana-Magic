...
ki.explode=function(pos,node)
	...
	node.place= node.place or {"ki:fire","air","air","air","air"}
...
if node.hurt==1 then
	for _, ob in ipairs(minetest.get_objects_inside_radius(pos, node.radius*2)) do
		if not (ob:get_luaentity() and (ob:get_luaentity().itemstring or ob:get_luaentity().ki_dust)) then
...
if node.velocity==1 then
	for _, ob in ipairs(minetest.get_objects_inside_radius(pos, node.radius*2)) do
		...
		local d=math.max(1,vector.distance(pos,pos2))
		local dmg=(8/d)*node.radius
		if ob:get_luaentity() and not ob:get_luaentity().attachplayer and not (ob:get_luaentity().ki_dust and ob:get_luaentity().nitroglycerine_dust==2) then
			ob:set_velocity({x=(pos2.x-pos.x)*dmg, y=(pos2.y-pos.y)*dmg, z=(pos2.z-pos.z)*dmg})


			if ob:get_luaentity() and ob:get_luaentity().ki_dust then ob:get_luaentity().ki=2 end

		elseif ob:is_player() then
			ki.new_player=ob
			minetest.add_entity({x=pos2.x,y=pos2.y+1,z=pos2.z}, "ki:playerp"):set_velocity({x=(pos2.x-pos.x)*dmg, y=(pos2.y-pos.y)*dmg, z=(pos2.z-pos.z)*dmg})
			ki.new_player=nil
		end
	end
end

	minetest.sound_play("ki_explode", {pos=pos, gain = 0.5, max_hear_distance = node.radius*8})
	if node.radius>9 then
		minetest.sound_play("ki", {pos=pos, gain = 0.5, max_hear_distance = node.radius*30})
	end
...
ki.freeze=function(ob)
	...
	if ob:is_player() then
		...
		minetest.set_node(pos, {name = "ki:icebox"})
	...
	if p.visual=="mesh" and p.mesh~="" and p.mesh~=nil and ob:get_luaentity().name~="ki:ice" then
		ki.newice=true
		local m=minetest.add_entity(pos, "ki:ice")
	...
	elseif ob:get_luaentity().name~="ki:ice" then
...
ki.spawn_dust=function(pos)
	...
	ki.new_dust={t=tx,drop=drop}
	minetest.add_entity(pos, "ki:dust")

	ki.new_dust=nil
end

minetest.register_entity("ki:ice",{
	hp_max = 1,
	physical = true,
	weight = 5,
...
minetest.register_entity("ki:dust",{
	hp_max = 1000,
	physical =true,
	weight = 0,
	...
	on_activate=function(self, staticdata)
		if not ki.new_dust then self.object:remove() return self end
		self.drop=ki.new_dust.drop
		self.object:set_properties({textures = ki.new_dust.t})
		self.object:set_acceleration({x=0,y=-10,z=0})
		return self
	end,
	on_step=function(self, dtime)
...
minetest.register_entity("ki:playerp",{
	hp_max = 1000,
	physical =true,
	...
	on_activate=function(self, staticdata)
		if not ki.new_player or minetest.check_player_privs(ki.new_player:get_player_name(), {fly=true}) then self.object:remove() return self end
		self.ob=ki.new_player
...
minetest.register_node("ki:icebox", {
	description = "Ice box",
...
minetest.register_node("ki:fire", {
	description = "fire",
...
minetest.register_node("ki:fire2", {
	description = "fire",
	inventory_image = "fire_basic_flame.png",
	drawtype = "firelike",
	tiles = {
...
minetest.register_abm({
	nodenames = {"ki:fire","ki:fire2"},
	interval = 1.0,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		local n=minetest.get_node(pos).name
		if n=="ki:fire" and math.random(1,20)==1 then
			minetest.set_node(pos, {name = "air"})
		elseif n=="ki:fire2" and math.random(1,5)==1 then
			minetest.set_node(pos, {name = "air"})
		end
	end,
})
